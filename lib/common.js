...
const indexDocs = async (app) => {
  ...
  const pages = docs.filter((doc) => doc.endsWith('.md'));
  ...
  for(let i = 0, len = pages.length; i < len; i++){
    ...
    const docDir = getDir(doc, docTitle);
  ...
  // Keep static docs
  console.log('Indexed docs: ', indexedDocs);
...
const getDir = (doc, docTitle) => {
  const exp = new RegExp(`^docs/(.*)/${docTitle}`);
  const result = exp.exec(doc);
